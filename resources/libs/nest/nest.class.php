...
//user_brightness
        ...
        $infos = (object) array(
            'current_state' => (object) array(
                ...
                'alt_heat' => $this->last_status->shared->{$serial_number}->hvac_aux_heater_state,
		...
		'aux_threshold' => $this->last_status->device->{$serial_number}->heat_pump_aux_threshold,
		'has_aux' => $this->last_status->device->{$serial_number}->has_aux_heat,
		'hvac_wires' => $this->last_status->device->{$serial_number}->hvac_wires,
            ),
            'target' => (object) array(
                'mode' => $target_mode,
